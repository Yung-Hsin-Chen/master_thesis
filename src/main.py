...
def main(experiment_version, fine_tuned=None):
    ...
    for cv in ["cv1", "cv2", "cv3", "cv4"]:
        ...
        val_wer_score, val_cer_score = train_charbert_trocr(experiment_version=experiment_version, train_loader=train_loader, val_loader=val_loader, device=device, fine_tuned=fine_tuned)
